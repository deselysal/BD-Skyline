...
def simulate_tree_gillespie(model, max_time=np.inf, min_sampled=0, max_sampled=np.inf,
    ...
    while infectious_nums.sum() and sampled_nums.sum() < target_sampled and time < max_time:
        # Get the current model based on current time
        if isinstance(model, BirthDeathSkylineModel):
            current_model = model.select_model(time)
            # Get the next model transition time
            next_model_change = float('inf')
            for model_params in model.ModelsList:
                if model_params["t"] > time:
                    next_model_change = min(next_model_change, model_params["t"])
                    break
        else:
            current_model = model
            next_model_change = float('inf')
        ...
        transmission_probs = current_model.transmission_rates / transmission_rates_per_state.reshape((num_states, 1))
        ...
        # Check if we need to change models before the next event
        if next_event_time > next_model_change:
